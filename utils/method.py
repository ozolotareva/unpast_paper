...
def run_MCL(
    similarity, inflations=list(np.arange(11, 30) / 10) + [3, 3.5, 4, 5], verbose=True
):
    if similarity.shape[0] == 0:
        return [[], []]
    ...
    for inflation in inflations:
        result = mc.run_mcl(
            sparse_matrix, inflation=inflation
    ...
    for mid in range(0, len(modules)):
        ...
        if len(genes) >= min_n_genes:
            ...
            if len(bicluster) > 0:
                ...
                bic = data_to_cluster.loc[genes, :].iloc[
                    :, sorted(bicluster["sample_indexes"])
                ]
                bg = data_to_cluster.loc[genes, :].iloc[
                    :,
                    [
                        ndx
                        for ndx in range(data_to_cluster.shape[1])
                        if not ndx in bicluster["sample_indexes"]
                    ],
                ]
                bicluster["genes_up"] = bic[
                    bic.mean(axis=1) >= bg.mean(axis=1)
                ].index.values
                bicluster["genes_down"] = bic[
                    bic.mean(axis=1) < bg.mean(axis=1)
                ].index.values

                if len(bicluster["genes_up"]) > 0 and len(bicluster["genes_down"]) > 0:
                    bic_profile = data_to_cluster.loc[bicluster["genes_up"], :].mean(
                        axis=0
                    ) - data_to_cluster.loc[sorted(bicluster["genes_down"]), :].mean(
                        axis=0
                    )
                else:
                    bic_profile = data_to_cluster.loc[bicluster["genes"], :].mean(
                        axis=0
                    )
                bicluster2 = cluster_samples(
                    bic_profile.T.values.reshape(-1, 1),
...
def make_consensus_biclusters(
    ...
    for i in range(n_runs):
        ...
        # biclusters_list[i].index = [str(x)+"_"+str(i) for x in biclusters_list[i].index.values]
