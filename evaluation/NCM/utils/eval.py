...
def generate_exprs(data_sizes, g_size=5, frac_samples=[0.05, 0.1, 0.25, 0.5],
    ...
    for s_frac in frac_samples:
        ...
        bic_samples = list(np.random.choice(
            list(bg_s), size=s_size, replace=False))
        bic_g += bic_genes
        bic_s += bic_samples
    ...
    for module in add_coexpressed:
        module_genes = list(np.random.choice(
        ...
        for i in biclusters.index.values:
            ...
            if pval.right_tail < pval.left_tail:
                ...
                if len(bic_members) > len(group_members):
                    ...
                    # compute J for bicluster and group complement
                ...
                jaccards[group][i] = 0 if union_complement == 0 else (
                    shared_complement/union_complement)

        # print(group,jaccards[group])

    pvals = pd.DataFrame.from_dict(pvals).loc[:, sorted_group_names]
    is_enriched = pd.DataFrame.from_dict(
        is_enriched).loc[:, sorted_group_names]
...
def find_best_matches(biclusters, known_groups, all_elements, FDR=0.05,
    ...
    try:
        ...
        evaluate_overlaps(biclusters, known_groups,
