...
result_file = '/Users/fernando/Documents/Research/DESMOND2/evaluation/clustering/results/OPTICS/clusters/A.n_genes=500,m=4,std=1,overlap=no_run1.tsv'
'''

df = pd.read_csv(input_file, sep='\t', index_col=0).T
distance_metrics = ['cityblock', 'cosine', 'euclidean', 'l1', 'l2', 'manhattan', 'braycurtis', 'canberra', 'chebyshev',
    ...
    for eps in epss:
        for cluster_method in cluster_methods:
            for algorithm in algorithms:
                result_filename = result_file.replace('.tsv',  f'_eps_{eps}_metric_{metric}_algorithm_{algorithm}_clustermethod_{re.sub("_", "", cluster_method)}_seed_{seed}.tsv')
                if not os.path.exists(result_filename):
                    try:
                        optic = OPTICS(min_samples=1, eps=eps, metric=metric, algorithm=algorithm, cluster_method=cluster_method).fit(df)
                        result_k = reformat_cluster_results(clusters=pd.DataFrame({'sample': df.index, 'label': optic.labels_}), input_df=df)
