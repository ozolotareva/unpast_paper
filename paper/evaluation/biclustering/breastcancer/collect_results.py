...
def get_overall_best_performer(summary_df, tool_path):
    ...
    try:
        summary_df.groupby('parameters')["overall_performance"].mean().sort_values(ascending=False).to_csv(out_file, sep='\t')
    except:
...
def collect(out_dir):
    ...
    for tool in os.listdir(out_dir):
        ...
        if os.path.isdir(tool_path):
                ...
                with open(os.path.join(tool_path, f'summary_{tool}_{config}.txt'), 'w') as fw:
                    ...
                    for result in os.listdir(config_path):
                        ...
                        if result.endswith("-scores_df.tsv"):
                            ...
                            scenario_params = result[len(parts[0])+1:len(underscore_splits[0])+1+len(underscore_splits[1])]
                            scenario_params_sep = scenario_params.split(',')
                            try:
                                param_part = result[len(parts[0])+len(scenario_params)+2:len(result)-14] if '_pre' not in result else result[len(result.split("_pre")[0])+1:len(result)-14]
                                d = {"scenario": parts[0], "run": run, 'gsize': int(scenario_params_sep[0].split('=')[1]),
                                     'parameters': param_part.replace('-', ';')}
                            except:
                                d = {"scenario": parts[0], "run": run, 'gsize': int(scenario_params_sep[0].split('=')[1]),
                                     'parameters': 'default'}
                            J_total = 0.0
                            performance = dict()
                            try:
                                best_matches = pd.read_csv(os.path.join(config_path, result), sep="\t", index_col=0, header=0)
            ...
            summary_df.to_csv(os.path.join(tool_path, 'summary-df.tsv'), sep="\t", index=False)
